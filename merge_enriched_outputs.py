...
def _merge_graphs(graph_files: Iterable[Path], doc_ids: Iterable[str]) -> Dict[str, Any]:
...
def merge_outputs(enriched_dir: Path, output_prefix: str = "merged") -> Dict[str, Path]:
    ...
    merged_graph = _merge_graphs(graph_files, doc_ids)
...
def main() -> None:
    ...
    outputs = merge_outputs(Path(args.enriched_dir), args.output_prefix)
